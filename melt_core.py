#!/usr/bin/env python3
"""
MeltTrafego - Núcleo do Sistema Multiplataforma
...
class MeltTrafegoCore:
    ...
    def verificar_dependencias(self):
        ...
        try:
            # Verificar tcpdump (Linux) ou npcap (Windows)
            if self.sistema == "Linux":
                ...
                # Verificar se Npcap está instalado verificando arquivos comuns
                npcap_paths = [
                    r"C:\Program Files\Npcap",
                    r"C:\Program Files (x86)\Npcap"
                ]
                dependencias['tcpdump'] = any(os.path.exists(path) for path in npcap_paths)
            ...
            dependencias['pandas'] = importlib.util.find_spec("pandas") is not None
    ...
    def listar_interfaces(self):
        ...
        try:
            if self.sistema == "Linux":
                ...
                # Usar scapy para listar interfaces no Windows
    ...
    def capturar_trafego(self, interface="any", tempo=60):
        ...
        try:
            if self.sistema == "Linux":
                ...
                # Usar scapy diretamente no Windows
                from scapy.all import sniff
                print(f"🎯 Capturando tráfego no Windows por {tempo}s...")
                
                pacotes = sniff(timeout=tempo, iface=interface)
                
                with open(arquivo_saida, 'w') as arquivo:
                    for pacote in pacotes:
                        arquivo.write(str(pacote) + '\n')
            ...
            if self.sistema == "Linux":
                ...
                print("❌ Erro: Npcap não encontrado. Instale o Npcap:")
    ...
    def monitorar_tempo_real(self, interface="any", duracao=30):
        ...
        try:
            if self.sistema == "Linux":
                ...
                from scapy.all import sniff
                print("📊 Monitoramento em tempo real no Windows...")
                
                def processar_pacote(pacote):
                    print(f"📦 {pacote.summary()}")
                
                sniff(prn=processar_pacote, timeout=duracao, iface=interface)
